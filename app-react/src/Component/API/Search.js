...
const Search = () => {
    ...
    const [inputS, setInput ] = useState("doremon")
    useEffect(() => {
        const getData = async () => {
            const res = await axios.get(`https://api.themoviedb.org/3/search/movie?query=${inputS}&api_key=c71d19f6720ccfe40c7918a3cc5626b4`)
            if (res.data.results) {
    ...
    return (
        <div className='p-10'>
            <div className="w-full max-w-[400px] mx-auto mb-20">
                <input
                    type="text"
                    className='w-full p-3 rounded-lg border border-purple-600 shadow-[0px_0px_0px_3px_rgba(125,_106,_255,_0.3)]'
                    value= {inputS}
            ...
            <div className='grid grid-cols-4 gap-5'>
                {movie.length > 0 && movie.map((item, index) => <MovieItem key={item.id} data={item}></MovieItem>)}
